...
import { viewAllCommentary } from "../../../assets/jsonData/JsonData";
...
export const ViewAllCommentary = () => {
  ...
  const columns =
    [
      ...
      {
        name: "Comment Title",
        selector: (row) => row.comment_title,
        sortable: true,
      },
      {
        name: "Description",
      ...
      {
        name: "Image",
        selector: (row) => <img className="h-6 w-6" src={row.image} />,
        sortable: true,
      },
      {
        name: "Status",
  ...
  return (
    <div className="flex flex-[0.85] flex-col overflow-auto">
      <ModuleHeader module={"Market Commentary"} subModule={"View All Commentary"} />
      <section className="h-full w-full flex-1 bg-white px-2 pt-2 pb-16 border-[12px] border-bgModulePage overflow-auto scrollBar-sm">
        ...
        <div className="w-full">
          <Table data={viewAllCommentary} columns={columns} customStyles={customStyles} />
        </div>
      </section>
    </div >
  );
